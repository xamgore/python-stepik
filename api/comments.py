...
class ListOfComments:
    ...
    def get(self, id: int) -> Comment:
        return Comment(self._stepik, self._stepik._fetch_object(Comment, id))


    def get_all(self, ids: List[int], keep_order=False) -> Iterable[Comment]:
    ...
    def create(self, target: str, parent: str = None, text: str = None, is_pinned: bool = None, is_reported: bool = None, attachments: str = None, thread: str = None, submission: str = None) -> Comment:
        vars = locals().copy()
        data = {'comment': {k: v for k, v in vars.items() if k != 'self' and v is not None}}

        resources_name = 'comments'
        response = self._stepik._post(resources_name, data)

        if resources_name not in response:
            raise StepikError(response)

        return Comment(self._stepik, response[resources_name][0])
