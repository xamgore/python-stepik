...
class ListOfReviews:
    ...
    def get(self, id: int) -> Review:
        return Review(self._stepik, self._stepik._fetch_object(Review, id))


    def get_all(self, ids: List[int], keep_order=False) -> Iterable[Review]:
        objects = self._stepik._fetch_objects(Review, ids)
        iterable = (Review(self._stepik, o) for o in objects)
    ...
    def create(self, session: str, target_session: str = None, text: str = None, rubric_scores: str = None) -> Review:
        vars = locals().copy()
        data = {'review': {k: v for k, v in vars.items() if k != 'self' and v is not None}}

        resources_name = 'reviews'
        response = self._stepik._post(resources_name, data)

        if resources_name not in response:
            raise StepikError(response)

        return Review(self._stepik, response[resources_name][0])
