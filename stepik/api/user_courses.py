...
class ListOfUserCourses:
    ...
    def get(self, id: int) -> UserCourse:
        obj = self._stepik._fetch_object(UserCourse, id)
        return UserCourse(self._stepik, obj)


    def get_all(self, ids: Iterable[int], keep_order=False) -> Iterable[UserCourse]:
        """
    ...
    def iterate(self,
                by_course: bool = None,
                by_id: bool = None,
                by_is_favorite: bool = None,
                by_last_viewed: bool = None,
                skip: int = 0, limit: Optional[int] = 20) -> Iterable[UserCourse]:
        """
        ...
        while True:
            page = self._stepik._get(f'user-courses?{params}&page={page_idx}&order={ordering}')

            for obj in page['user-courses']:
                if limit and count >= limit:
                    break

                yield UserCourse(self._stepik, obj)
                count += 1
