...
class ListOfMembers:
    ...
    def get(self, id: int) -> Member:
        obj = self._stepik._fetch_object(Member, id)
        return Member(self._stepik, obj)


    def get_all(self, ids: Iterable[int], keep_order=False) -> Iterable[Member]:
        """
    ...
    def iterate(self,
                skip: int = 0, limit: Optional[int] = 20) -> Iterable[Member]:
        """
        ...
        while True:
            page = self._stepik._get(f'members?{params}&page={page_idx}&order={ordering}')

            for obj in page['members']:
                if limit and count >= limit:
                    break

                yield Member(self._stepik, obj)
                count += 1
    ...
    def create(self,
        ...
        response = self._stepik._post('members', data)
        if 'members' not in response:
            raise StepikError(response)

        return Member(self._stepik, response['members'][0])
