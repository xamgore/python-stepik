...
class ListOfCourseReminders:
    ...
    def get(self, id: int) -> CourseReminder:
        obj = self._stepik._fetch_object(CourseReminder, id)
        return CourseReminder(self._stepik, obj)


    def get_all(self, ids: Iterable[int], keep_order=False) -> Iterable[CourseReminder]:
        """
    ...
    def iterate(self,
                skip: int = 0, limit: Optional[int] = 20) -> Iterable[CourseReminder]:
        """
        ...
        while True:
            page = self._stepik._get(f'course-reminders?{params}&page={page_idx}&order={ordering}')

            for obj in page['course-reminders']:
                if limit and count >= limit:
                    break

                yield CourseReminder(self._stepik, obj)
                count += 1
    ...
    def create(self,
               course: str,
               is_active: bool = None,
               **kwargs) -> CourseReminder:
        vars = locals().copy()
        data = {'course-reminder':
                    {k: v for k, v in {**vars, **kwargs}.items()
                     if k != 'self' and v is not None}}

        response = self._stepik._post('course-reminders', data)
