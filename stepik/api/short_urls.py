...
class ShortUrl:
    """
    Current shortifyer's events resource.
    """
...
class ListOfShortUrls:
    ...
    def get_all(self, ids: Iterable[int], keep_order=False) -> Iterable[ShortUrl]:
        """
    ...
    def iterate(self,
                key: str = None,
                skip: int = 0, limit: Optional[int] = 20) -> Iterable[ShortUrl]:
        """
        ...
        while True:
            page = self._stepik._get(f'short-urls?{params}&page={page_idx}&order={ordering}')

            for obj in page['short-urls']:
                if limit and count >= limit:
                    break

                yield ShortUrl(self._stepik, obj)
                count += 1
    ...
    def create(self,
        ...
        response = self._stepik._post('short-urls', data)
        if 'short-urls' not in response:
            raise StepikError(response)

        return ShortUrl(self._stepik, response['short-urls'][0])
