...
class ListOfPlatformNews:
    ...
    def get(self, id: int) -> PlatformNews:
        obj = self._stepik._fetch_object(PlatformNews, id)
        return PlatformNews(self._stepik, obj)


    def get_all(self, ids: Iterable[int], keep_order=False) -> Iterable[PlatformNews]:
        """
    ...
    def iterate(self,
                skip: int = 0, limit: Optional[int] = 20) -> Iterable[PlatformNews]:
        """
        ...
        while True:
            page = self._stepik._get(f'platform-news?{params}&page={page_idx}&order={ordering}')

            for obj in page['platform-news']:
                if limit and count >= limit:
                    break

                yield PlatformNews(self._stepik, obj)
                count += 1
