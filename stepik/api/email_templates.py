...
class ListOfEmailTemplates:
    ...
    def get(self, id: int) -> EmailTemplate:
        obj = self._stepik._fetch_object(EmailTemplate, id)
        return EmailTemplate(self._stepik, obj)


    def get_all(self, ids: Iterable[int], keep_order=False) -> Iterable[EmailTemplate]:
        """
    ...
    def iterate(self,
                ...
                mail_type: str = None,
                course: Any = None,
                skip: int = 0, limit: Optional[int] = 20) -> Iterable[EmailTemplate]:
        """
        ...
        while True:
            page = self._stepik._get(f'email-templates?{params}&page={page_idx}&order={ordering}')

            for obj in page['email-templates']:
                if limit and count >= limit:
                    break

                yield EmailTemplate(self._stepik, obj)
                count += 1
