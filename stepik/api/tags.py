...
class ListOfTags:
    ...
    def get(self, id: int) -> Tag:
        obj = self._stepik._fetch_object(Tag, id)
        return Tag(self._stepik, obj)


    def get_all(self, ids: Iterable[int], keep_order=False) -> Iterable[Tag]:
        """
    ...
    def iterate(self,
                skip: int = 0, limit: Optional[int] = 20) -> Iterable[Tag]:
        """
        ...
        while True:
            page = self._stepik._get(f'tags?{params}&page={page_idx}&order={ordering}')

            for obj in page['tags']:
                if limit and count >= limit:
                    break

                yield Tag(self._stepik, obj)
                count += 1
