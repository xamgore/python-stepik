...
class ListOfAchievements:
    ...
    def get(self, id: int) -> Achievement:
        obj = self._stepik._fetch_object(Achievement, id)
        return Achievement(self._stepik, obj)


    def get_all(self, ids: Iterable[int], keep_order=False) -> Iterable[Achievement]:
        """
    ...
    def iterate(self,
                skip: int = 0, limit: Optional[int] = 20) -> Iterable[Achievement]:
        """
        ...
        while True:
            page = self._stepik._get(f'achievements?{params}&page={page_idx}&order={ordering}')

            for obj in page['achievements']:
                if limit and count >= limit:
                    break

                yield Achievement(self._stepik, obj)
                count += 1
