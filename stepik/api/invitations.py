...
class ListOfInvitations:
    ...
    def get(self, id: int) -> Invitation:
        obj = self._stepik._fetch_object(Invitation, id)
        return Invitation(self._stepik, obj)


    def get_all(self, ids: Iterable[int], keep_order=False) -> Iterable[Invitation]:
        """
    ...
    def iterate(self,
                skip: int = 0, limit: Optional[int] = 20) -> Iterable[Invitation]:
        """
        ...
        while True:
            page = self._stepik._get(f'invitations?{params}&page={page_idx}&order={ordering}')

            for obj in page['invitations']:
                if limit and count >= limit:
                    break

                yield Invitation(self._stepik, obj)
                count += 1
    ...
    def create(self,
        ...
        response = self._stepik._post('invitations', data)
        if 'invitations' not in response:
            raise StepikError(response)

        return Invitation(self._stepik, response['invitations'][0])
