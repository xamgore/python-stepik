...
class ListOfCities:
    ...
    def get(self, id: int) -> City:
        obj = self._stepik._fetch_object(City, id)
        return City(self._stepik, obj)


    def get_all(self, ids: Iterable[int], keep_order=False) -> Iterable[City]:
        """
    ...
    def iterate(self,
                skip: int = 0, limit: Optional[int] = 20) -> Iterable[City]:
        """
        ...
        while True:
            page = self._stepik._get(f'cities?{params}&page={page_idx}&order={ordering}')

            for obj in page['cities']:
                if limit and count >= limit:
                    break

                yield City(self._stepik, obj)
                count += 1
