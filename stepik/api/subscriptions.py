...
class ListOfSubscriptions:
    ...
    def get(self, id: str) -> Subscription:
        obj = self._stepik._fetch_object(Subscription, id)
        return Subscription(self._stepik, obj)


    def get_all(self, ids: Iterable[str], keep_order=False) -> Iterable[Subscription]:
        """
    ...
    def iterate(self,
                skip: int = 0, limit: Optional[int] = 20) -> Iterable[Subscription]:
        """
        ...
        while True:
            page = self._stepik._get(f'subscriptions?{params}&page={page_idx}&order={ordering}')

            for obj in page['subscriptions']:
                if limit and count >= limit:
                    break

                yield Subscription(self._stepik, obj)
                count += 1
