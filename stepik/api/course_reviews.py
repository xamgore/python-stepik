...
class ListOfCourseReviews:
    ...
    def get(self, id: int) -> CourseReview:
        obj = self._stepik._fetch_object(CourseReview, id)
        return CourseReview(self._stepik, obj)


    def get_all(self, ids: Iterable[int], keep_order=False) -> Iterable[CourseReview]:
        """
    ...
    def iterate(self,
                ...
                by_score: bool = None,
                skip: int = 0, limit: Optional[int] = 20) -> Iterable[CourseReview]:
        """
        ...
        while True:
            page = self._stepik._get(f'course-reviews?{params}&page={page_idx}&order={ordering}')

            for obj in page['course-reviews']:
                if limit and count >= limit:
                    break

                yield CourseReview(self._stepik, obj)
                count += 1
    ...
    def create(self,
        ...
        response = self._stepik._post('course-reviews', data)
