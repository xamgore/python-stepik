...
class ListOfAchievementProgresses:
    ...
    def get(self, id: int) -> AchievementProgress:
        obj = self._stepik._fetch_object(AchievementProgress, id)
        return AchievementProgress(self._stepik, obj)


    def get_all(self, ids: Iterable[int], keep_order=False) -> Iterable[AchievementProgress]:
        """
    ...
    def iterate(self,
                user: Any = None,
                achievement: Any = None,
                kind: Any = None,
                is_obtained: bool = None,
                by_create_date: bool = None,
                by_id: bool = None,
                by_update_date: bool = None,
                by_obtain_date: bool = None,
                by_score: bool = None,
                skip: int = 0, limit: Optional[int] = 20) -> Iterable[AchievementProgress]:
        """
        ...
        while True:
            page = self._stepik._get(f'achievement-progresses?{params}&page={page_idx}&order={ordering}')

            for obj in page['achievement-progresses']:
                if limit and count >= limit:
                    break

                yield AchievementProgress(self._stepik, obj)
                count += 1
