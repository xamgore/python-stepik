...
class ListOfAssignments:
    ...
    def get(self, id: int) -> Assignment:
        obj = self._stepik._fetch_object(Assignment, id)
        return Assignment(self._stepik, obj)


    def get_all(self, ids: Iterable[int], keep_order=False) -> Iterable[Assignment]:
        """
        ...
        objects = self._stepik._fetch_objects(Assignment, ids)
        iterable = (Assignment(self._stepik, o) for o in objects)
    ...
    def iterate(self,
                skip: int = 0, limit: Optional[int] = 20) -> Iterable[Assignment]:
        """
        ...
        while True:
            page = self._stepik._get(f'assignments?{params}&page={page_idx}&order={ordering}')

            for obj in page['assignments']:
                if limit and count >= limit:
                    break

                yield Assignment(self._stepik, obj)
                count += 1
