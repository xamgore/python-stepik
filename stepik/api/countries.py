...
class ListOfCountries:
    ...
    def get(self, id: int) -> Country:
        obj = self._stepik._fetch_object(Country, id)
        return Country(self._stepik, obj)


    def get_all(self, ids: Iterable[int], keep_order=False) -> Iterable[Country]:
        """
    ...
    def iterate(self,
                skip: int = 0, limit: Optional[int] = 20) -> Iterable[Country]:
        """
        ...
        while True:
            page = self._stepik._get(f'countries?{params}&page={page_idx}&order={ordering}')

            for obj in page['countries']:
                if limit and count >= limit:
                    break

                yield Country(self._stepik, obj)
                count += 1
