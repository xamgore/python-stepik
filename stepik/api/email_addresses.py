...
class ListOfEmailAddresses:
    ...
    def get(self, id: int) -> EmailAddress:
        obj = self._stepik._fetch_object(EmailAddress, id)
        return EmailAddress(self._stepik, obj)


    def get_all(self, ids: Iterable[int], keep_order=False) -> Iterable[EmailAddress]:
        """
    ...
    def iterate(self,
                ...
                skip: int = 0, limit: Optional[int] = 20) -> Iterable[EmailAddress]:
        """
        ...
        while True:
            page = self._stepik._get(f'email-addresses?{params}&page={page_idx}&order={ordering}')

            for obj in page['email-addresses']:
                if limit and count >= limit:
                    break

                yield EmailAddress(self._stepik, obj)
                count += 1
    ...
    def create(self,
        ...
        response = self._stepik._post('email-addresses', data)
        if 'email-addresses' not in response:
            raise StepikError(response)

        return EmailAddress(self._stepik, response['email-addresses'][0])
