...
class ListOfRubricScores:
    def __init__(self, stepik):
        from stepik import Stepik
        self._stepik: Stepik = stepik


    def get(self, id: int) -> RubricScore:
        obj = self._stepik._fetch_object(RubricScore, id)
        return RubricScore(self._stepik, obj)


    def get_all(self, ids: Iterable[int], keep_order=False) -> Iterable[RubricScore]:
        """
        ...
        objects = self._stepik._fetch_objects(RubricScore, ids)
        iterable = (RubricScore(self._stepik, o) for o in objects)
    ...
    def iterate(self,
                skip: int = 0, limit: Optional[int] = 20) -> Iterable[RubricScore]:
        """
        ...
        while True:
            page = self._stepik._get(f'rubric-scores?{params}&page={page_idx}&order={ordering}')

            for obj in page['rubric-scores']:
                if limit and count >= limit:
                    break

                yield RubricScore(self._stepik, obj)
                count += 1
