...
class ListOfClassPlans:
    ...
    def get(self, id: int) -> ClassPlan:
        obj = self._stepik._fetch_object(ClassPlan, id)
        return ClassPlan(self._stepik, obj)


    def get_all(self, ids: Iterable[int], keep_order=False) -> Iterable[ClassPlan]:
        """
    ...
    def iterate(self,
                skip: int = 0, limit: Optional[int] = 20) -> Iterable[ClassPlan]:
        """
        ...
        while True:
            page = self._stepik._get(f'class-plans?{params}&page={page_idx}&order={ordering}')

            for obj in page['class-plans']:
                if limit and count >= limit:
                    break

                yield ClassPlan(self._stepik, obj)
                count += 1
