...
class ListOfProctorSessions:
    ...
    def get(self, id: int) -> ProctorSession:
        obj = self._stepik._fetch_object(ProctorSession, id)
        return ProctorSession(self._stepik, obj)


    def get_all(self, ids: Iterable[int], keep_order=False) -> Iterable[ProctorSession]:
        """
    ...
    def iterate(self,
                ...
                course: Any = None,
                status: Any = None,
                skip: int = 0, limit: Optional[int] = 20) -> Iterable[ProctorSession]:
        """
        ...
        while True:
            page = self._stepik._get(f'proctor-sessions?{params}&page={page_idx}&order={ordering}')

            for obj in page['proctor-sessions']:
                if limit and count >= limit:
                    break

                yield ProctorSession(self._stepik, obj)
                count += 1
    ...
    def create(self,
               section: str,
               **kwargs) -> ProctorSession:
        vars = locals().copy()
        data = {'proctor-session':
                    {k: v for k, v in {**vars, **kwargs}.items()
                     if k != 'self' and v is not None}}

        response = self._stepik._post('proctor-sessions', data)
        if 'proctor-sessions' not in response:
            raise StepikError(response)

        return ProctorSession(self._stepik, response['proctor-sessions'][0])
