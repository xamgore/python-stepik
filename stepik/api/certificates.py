...
class ListOfCertificates:
    ...
    def get(self, id: int) -> Certificate:
        obj = self._stepik._fetch_object(Certificate, id)
        return Certificate(self._stepik, obj)


    def get_all(self, ids: Iterable[int], keep_order=False) -> Iterable[Certificate]:
        """
    ...
    def iterate(self,
                by_id: bool = None,
                skip: int = 0, limit: Optional[int] = 20) -> Iterable[Certificate]:
        """
        ...
        while True:
            page = self._stepik._get(f'certificates?{params}&page={page_idx}&order={ordering}')

            for obj in page['certificates']:
                if limit and count >= limit:
                    break

                yield Certificate(self._stepik, obj)
                count += 1
