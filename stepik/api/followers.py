...
class ListOfFollowers:
    ...
    def get(self, id: int) -> Follower:
        obj = self._stepik._fetch_object(Follower, id)
        return Follower(self._stepik, obj)


    def get_all(self, ids: Iterable[int], keep_order=False) -> Iterable[Follower]:
        """
    ...
    def iterate(self,
                skip: int = 0, limit: Optional[int] = 20) -> Iterable[Follower]:
        """
        ...
        while True:
            page = self._stepik._get(f'followers?{params}&page={page_idx}&order={ordering}')

            for obj in page['followers']:
                if limit and count >= limit:
                    break

                yield Follower(self._stepik, obj)
                count += 1
