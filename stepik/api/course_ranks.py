...
class ListOfCourseRanks:
    ...
    def get(self, id: int) -> CourseRank:
        obj = self._stepik._fetch_object(CourseRank, id)
        return CourseRank(self._stepik, obj)


    def get_all(self, ids: Iterable[int], keep_order=False) -> Iterable[CourseRank]:
        """
    ...
    def iterate(self,
                skip: int = 0, limit: Optional[int] = 20) -> Iterable[CourseRank]:
        """
        ...
        while True:
            page = self._stepik._get(f'course-ranks?{params}&page={page_idx}&order={ordering}')

            for obj in page['course-ranks']:
                if limit and count >= limit:
                    break

                yield CourseRank(self._stepik, obj)
                count += 1
