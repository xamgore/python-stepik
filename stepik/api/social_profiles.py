...
class ListOfSocialProfiles:
    ...
    def get(self, id: int) -> SocialProfile:
        obj = self._stepik._fetch_object(SocialProfile, id)
        return SocialProfile(self._stepik, obj)


    def get_all(self, ids: Iterable[int], keep_order=False) -> Iterable[SocialProfile]:
        """
    ...
    def iterate(self,
                by_id: bool = None,
                skip: int = 0, limit: Optional[int] = 20) -> Iterable[SocialProfile]:
        """
        ...
        while True:
            page = self._stepik._get(f'social-profiles?{params}&page={page_idx}&order={ordering}')

            for obj in page['social-profiles']:
                if limit and count >= limit:
                    break

                yield SocialProfile(self._stepik, obj)
                count += 1
