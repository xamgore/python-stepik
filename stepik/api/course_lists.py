...
class ListOfCourseLists:
    ...
    def get(self, id: int) -> CourseList:
        obj = self._stepik._fetch_object(CourseList, id)
        return CourseList(self._stepik, obj)


    def get_all(self, ids: Iterable[int], keep_order=False) -> Iterable[CourseList]:
        """
    ...
    def iterate(self,
                language: str = None,
                platform: str = None,
                skip: int = 0, limit: Optional[int] = 20) -> Iterable[CourseList]:
        """
        ...
        while True:
            page = self._stepik._get(f'course-lists?{params}&page={page_idx}&order={ordering}')

            for obj in page['course-lists']:
                if limit and count >= limit:
                    break

                yield CourseList(self._stepik, obj)
                count += 1
