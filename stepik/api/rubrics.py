...
class ListOfRubrics:
    ...
    def get(self, id: int) -> Rubric:
        obj = self._stepik._fetch_object(Rubric, id)
        return Rubric(self._stepik, obj)


    def get_all(self, ids: Iterable[int], keep_order=False) -> Iterable[Rubric]:
        """
    ...
    def iterate(self,
                skip: int = 0, limit: Optional[int] = 20) -> Iterable[Rubric]:
        """
        ...
        while True:
            page = self._stepik._get(f'rubrics?{params}&page={page_idx}&order={ordering}')

            for obj in page['rubrics']:
                if limit and count >= limit:
                    break

                yield Rubric(self._stepik, obj)
                count += 1
    ...
    def create(self,
        ...
        response = self._stepik._post('rubrics', data)
