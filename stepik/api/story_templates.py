...
class ListOfStoryTemplates:
    ...
    def get(self, id: int) -> StoryTemplate:
        obj = self._stepik._fetch_object(StoryTemplate, id)
        return StoryTemplate(self._stepik, obj)


    def get_all(self, ids: Iterable[int], keep_order=False) -> Iterable[StoryTemplate]:
        """
    ...
    def iterate(self,
                skip: int = 0, limit: Optional[int] = 20) -> Iterable[StoryTemplate]:
        """
        ...
        while True:
            page = self._stepik._get(f'story-templates?{params}&page={page_idx}&order={ordering}')

            for obj in page['story-templates']:
                if limit and count >= limit:
                    break

                yield StoryTemplate(self._stepik, obj)
                count += 1
