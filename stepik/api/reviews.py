...
class ListOfReviews:
    ...
    def get(self, id: int) -> Review:
        obj = self._stepik._fetch_object(Review, id)
        return Review(self._stepik, obj)


    def get_all(self, ids: Iterable[int], keep_order=False) -> Iterable[Review]:
        """
    ...
    def iterate(self,
                skip: int = 0, limit: Optional[int] = 20) -> Iterable[Review]:
        """
        ...
        while True:
            page = self._stepik._get(f'reviews?{params}&page={page_idx}&order={ordering}')

            for obj in page['reviews']:
                if limit and count >= limit:
                    break

                yield Review(self._stepik, obj)
                count += 1
