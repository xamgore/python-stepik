...
class ListOfScoreFiles:
    ...
    def get(self, id: int) -> ScoreFile:
        obj = self._stepik._fetch_object(ScoreFile, id)
        return ScoreFile(self._stepik, obj)


    def get_all(self, ids: Iterable[int], keep_order=False) -> Iterable[ScoreFile]:
        """
    ...
    def iterate(self,
                ...
                skip: int = 0, limit: Optional[int] = 20) -> Iterable[ScoreFile]:
        """
        ...
        while True:
            page = self._stepik._get(f'score-files?{params}&page={page_idx}&order={ordering}')

            for obj in page['score-files']:
                if limit and count >= limit:
                    break

                yield ScoreFile(self._stepik, obj)
                count += 1
