...
class ListOfRecommendations:
    ...
    def get_all(self, ids: Iterable[str], keep_order=False) -> Iterable[Recommendation]:
        """
    ...
    def iterate(self,
                skip: int = 0, limit: Optional[int] = 20) -> Iterable[Recommendation]:
        """
        ...
        while True:
            page = self._stepik._get(f'recommendations?{params}&page={page_idx}&order={ordering}')

            for obj in page['recommendations']:
                if limit and count >= limit:
                    break

                yield Recommendation(self._stepik, obj)
                count += 1
