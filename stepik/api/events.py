...
class ListOfEvents:
    ...
    def get(self, id: int) -> Event:
        obj = self._stepik._fetch_object(Event, id)
        return Event(self._stepik, obj)


    def get_all(self, ids: Iterable[int], keep_order=False) -> Iterable[Event]:
        """
    ...
    def iterate(self,
                type: Any = None,
                skip: int = 0, limit: Optional[int] = 20) -> Iterable[Event]:
        """
        ...
        while True:
            page = self._stepik._get(f'events?{params}&page={page_idx}&order={ordering}')

            for obj in page['events']:
                if limit and count >= limit:
                    break

                yield Event(self._stepik, obj)
                count += 1
