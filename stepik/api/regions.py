...
class ListOfRegions:
    ...
    def get(self, id: int) -> Region:
        obj = self._stepik._fetch_object(Region, id)
        return Region(self._stepik, obj)


    def get_all(self, ids: Iterable[int], keep_order=False) -> Iterable[Region]:
        """
        ...
        objects = self._stepik._fetch_objects(Region, ids)
        iterable = (Region(self._stepik, o) for o in objects)
    ...
    def iterate(self,
                skip: int = 0, limit: Optional[int] = 20) -> Iterable[Region]:
        """
        ...
        while True:
            page = self._stepik._get(f'regions?{params}&page={page_idx}&order={ordering}')

            for obj in page['regions']:
                if limit and count >= limit:
                    break

                yield Region(self._stepik, obj)
                count += 1
