...
class ListOfFavoriteCourses:
    ...
    def get(self, id: int) -> FavoriteCourse:
        obj = self._stepik._fetch_object(FavoriteCourse, id)
        return FavoriteCourse(self._stepik, obj)


    def get_all(self, ids: Iterable[int], keep_order=False) -> Iterable[FavoriteCourse]:
        """
    ...
    def iterate(self,
                skip: int = 0, limit: Optional[int] = 20) -> Iterable[FavoriteCourse]:
        """
        ...
        while True:
            page = self._stepik._get(f'favorite-courses?{params}&page={page_idx}&order={ordering}')

            for obj in page['favorite-courses']:
                if limit and count >= limit:
                    break

                yield FavoriteCourse(self._stepik, obj)
                count += 1
