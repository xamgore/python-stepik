...
class ListOfStripeCoupons:
    def __init__(self, stepik):
        from stepik import Stepik
        self._stepik: Stepik = stepik


    def get_all(self, ids: Iterable[int], keep_order=False) -> Iterable[StripeCoupon]:
        """
        ...
        objects = self._stepik._fetch_objects(StripeCoupon, ids)
        iterable = (StripeCoupon(self._stepik, o) for o in objects)
    ...
    def iterate(self,
                skip: int = 0, limit: Optional[int] = 20) -> Iterable[StripeCoupon]:
        """
        ...
        while True:
            page = self._stepik._get(f'stripe-coupons?{params}&page={page_idx}&order={ordering}')

            for obj in page['stripe-coupons']:
                if limit and count >= limit:
                    break

                yield StripeCoupon(self._stepik, obj)
                count += 1
