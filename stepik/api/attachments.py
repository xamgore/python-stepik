...
class ListOfAttachments:
    ...
    def get(self, id: int) -> Attachment:
        obj = self._stepik._fetch_object(Attachment, id)
        return Attachment(self._stepik, obj)


    def get_all(self, ids: Iterable[int], keep_order=False) -> Iterable[Attachment]:
        """
    ...
    def iterate(self,
                course: Any = None,
                lesson: Any = None,
                skip: int = 0, limit: Optional[int] = 20) -> Iterable[Attachment]:
        """
        ...
        while True:
            page = self._stepik._get(f'attachments?{params}&page={page_idx}&order={ordering}')

            for obj in page['attachments']:
                if limit and count >= limit:
                    break

                yield Attachment(self._stepik, obj)
                count += 1
    ...
    def create(self,
        ...
        response = self._stepik._post('attachments', data)
